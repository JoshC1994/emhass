...
- `variable_timestep`: Set to `True` to preserve the actual timestamps from runtime forecast dictionaries and allow downstream processing to use an irregular forecast horizon instead of always rebuilding a uniform grid from `optimization_time_step`. Defaults to `False`. When left at `False`, EMHASS keeps the existing fixed-step behavior.
